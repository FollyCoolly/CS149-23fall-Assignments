...
void clampedExpVector(float *values, int *exponents, float *output, int N) {
  ...
  __cs149_mask maskAll, maskGTZero, maskOdd, maskClamp, maskNotClamp;
  __cs149_vec_float result;
  __cs149_vec_float base;
  __cs149_vec_int exp;
  __cs149_vec_int zero = _cs149_vset_int(0);
  __cs149_vec_int two = _cs149_vset_int(2);
  __cs149_vec_int round;
  __cs149_vec_float clamp = _cs149_vset_float(9.999999f);

  for (int i = 0; i  < N ; i += VECTOR_WIDTH) {
    maskAll = _cs149_init_ones(i + VECTOR_WIDTH <= N ? VECTOR_WIDTH : (N % VECTOR_WIDTH));
    maskGTZero = _cs149_init_ones(0);
    maskOdd = _cs149_init_ones(0);
    ...
    while (_cs149_cntbits(maskGTZero) > 0) {
      _cs149_vdiv_int(round, exp, two, maskAll);
      _cs149_vmult_int(round, round, two, maskAll);
      _cs149_veq_int(maskOdd, round, exp, maskAll);
      maskOdd = _cs149_mask_not(maskOdd);
      // if (e / 2 * 2 != e) res *= base
      ...
      // if (res < 9.999999f) base *= base
      maskNotClamp = _cs149_mask_not(maskClamp);
      _cs149_vmult_float(base, base, base, maskNotClamp);

      // e /= 2
      _cs149_vdiv_int(exp, exp, two, maskAll);
      _cs149_vgt_int(maskGTZero, exp, zero, maskAll);
    }
  ...
  // if(last != N){
  //   clampedExpSerial(values + last, exponents + last, output + last, N - last);
  // }
}
...
float arraySumVector(float *values, int N) {
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    ...
    _cs149_vadd_float(sum, sum, val, maskAll);
  }
